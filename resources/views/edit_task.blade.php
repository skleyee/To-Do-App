...
<div class="container">
    ...
    <div class="row justify-content-center">
        <form action="{{ route('update.task', ['id' => $task->id]) }}" class="col-md-8" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="form-group">
                @if(!empty($categories))
                    ...
                    <select name="category_id" class="form-control" id="category">
                        <option value="new" {{ $task->category_id == null ? 'selected' : '' }}>None</option>
                        @foreach($categories as $category)
                            <option {{ $task->category_id === $category->id ? 'selected' : '' }} value="{{ $category->id }}">{{ $category->title }}</option>
                        @endforeach
                    </select>
