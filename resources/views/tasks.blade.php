            ...
            <div class="col-md-12">
                <div class="card px-3">
                    <div class="card-body">
                        ...
                        <a style="color:white !important;" href="{{ route('create.task') }}" class="add btn btn-primary font-weight-bold todo-list-add-btn mt-3 mb-4">Add new task</a>
                        <div class="form-group">
                        <form action="{{ route('filter') }}" method="POST">
                            @csrf
                            <label for="categoryFilter">Filter by Category:</label>
                            <select name="category_id" class="form-control" id="categoryFilter">
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}">{{ $category->title }}</option>
                                @endforeach
                            </select>
                            <button type="submit" class="btn btn-primary mt-2">Apply filter</button>
                            <a href="{{route('tasks')}}" style="color: white !important;" class="btn btn-secondary mr-2 mt-2">Reset filter</a>
                        </form>
                        </div>
                                ...
                                <li class="{{ $task->is_complete ? 'completed' : '' }}">
                                    <div class="form-check">
                                        <label class="form-check-label">
                                            <input id="{{ $task->id }}" class="checkbox is_complete" name="is_complete" {{ $task->is_complete ? 'checked' : '' }} type="checkbox"> {{ $task->title }}
                                            <i id="{{ $task->id }}" class="input-helper"></i>
                                            <span class="badge badge-secondary ml-2">{{ $categories[$task->category_id]->title ?? '' }}</span>
                                        </label>
                                    </div>
                                    <div class="remove_block">
                                        <a href="{{ route('show.task', ['id' => $task->id]) }}" class="edit fa fa-pencil-square-o" aria-hidden="true"></a>
                                        <i id="{{ $task->id }}" class="remove mdi mdi-close-circle-outline"></i>
                                    </div>
                                </li>
                                @endforeach
                            </ul>
